...
ULONG cmd_run_notify(CMD_LINE_S *pstCmdLine, CMD_VTY_S *vty, CHAR **argv, ULONG argc)
{
	...
	CMD_NTF_NODE_S *pstHead = cmd_get_regntfhead();
...
CHAR vty_getch(CMD_VTY_S *vty)
{
	...
	//CMD_debug(CMD_DEBUG_INFO, "vty_getch. (c=0x%x)", buff);
...
ULONG cmd_parse_vty(CMD_VTY_S *vty)
{
	CHAR c = vty->c;
...
VOID cmd_read(CMD_VTY_S *vty)
{
	...
	while ((vty->c = vty_getch(vty)) > 0) {
		/* check vty is online */
		if (0 == vty->used) {
			CMD_debug(CMD_DEBUG_ERROR, "cmd_read, vty is not used. vtyId=%d", vty->vtyId);
			break;
		}
		...
		if (CMD_VTY_CONSOLE_ID == vty->vtyId) {
			ucKeyType = cmd_parse(vty);
		} else {
			ucKeyType = cmd_parse_vty(vty);
		}
		...
		if (ucKeyType <= CMD_KEY_CODE_NONE || ucKeyType > CMD_KEY_CODE_INSERT) {
			CMD_debug(CMD_DEBUG_ERROR, "Unknow key type, c=%c, ucKeyType=%u\n", vty->c, ucKeyType);
			continue;
		}

		g_cmd_key_handle[ucKeyType].pKeyCallbackfunc(vty);
...
int cmd_main_entry(void *pEntry)
{
	for (;;)
	{
		cmd_read(cmd_vty_getById(CMD_VTY_CONSOLE_ID));
	}
